...
public class ClassC extends row{
	...
	public ClassC(int Width, int level) {
		...
		ImageIcon icon = new ImageIcon("D:\\Users\\Gabo\\Escritorio\\Proyectos\\Invaders\\src\\images\\ClassC.png");
		this.setIcon(icon);
			...
			for (int i = 0; i < this.getLength(); i++) {
				...
				if(aux.getPosX() <= bulletx && bulletx <= (aux.getPosX()+aux.getSizeX())) {
					if (aux.getResistance() == 0) {
						int pos = this.list.position(aux);
